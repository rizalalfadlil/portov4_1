...
export function AboutSection({ ref }: { ref?: React.Ref<HTMLDivElement> }) {
  ...
  return (
    <section
      ref={ref}
      className="p-4 min-h-screen flex flex-col items-center py-20 space-y-20 justify-center bg-gray-100"
      id="About"
    >
      <div className="w-full max-w-5xl mx-auto text-center text-primary">
        <h2 className="text-3xl font-bold mb-2">About</h2>
        <h4 className="text-accent">
          Here’s a bit about who I am, what I do, and how you can connect with
          me.
        </h4>
      </div>
      <div className="w-full p-4 max-w-5xl mx-auto text-center grid sm:flex gap-8">
        ...
        <div className="text-start text-primary grid content-center sm:w-2/3">
          <p className="text-xl mb-2 font-bold">Hafidz Rizal Al-Fadlil</p>
          <p className="text-sm text-accent">
            Hi, I’m Hafidz Rizal Al-Fadill. I love turning complex ideas into
            simple, intuitive digital experiences. I’m always eager to learn,
            explore new technologies, and work on meaningful projects that
            create impact.
          </p>
        </div>
        <div className="text-accent sm:w-1/3 h-full flex flex-col gap-2">
...
function ExternalLinkButton({
  ...
  return (
    <div
      onClick={action}
      className="flex items-center gap-4 text-start p-4 rounded-sm hover:font-bold select-none cursor-pointer border transition-all"
    >
      {icon}
...
function Social({
  ...
  return (
    <div
      className="rounded-xl p-3 flex font-medium hover:outline hover:outline duration-300 hover:opacity-80 transition-all items-center gap-2 select-none cursor-pointer "
      onClick={() => {
  ...
  return (
    <div className=" max-h-80 overflow-y-scroll">
      {certificates.map((cert, index) => (
        <div
          key={index}
          className="p-2 hover:bg-gray-100 rounded-sm cursor-pointer transition-colors"
        >
